...
func _ready() -> void:
	...
	# Hide respawn button — not used in episode 3
	respawn_btn.visible = false
	respawn_btn.mouse_filter = Control.MOUSE_FILTER_IGNORE
	...
	reload_btn.position = Vector2((viewport_size.x - reload_w) / 2.0, btn_y)
	reload_btn.modulate = Color(1, 1, 1, 0)

	# Hover tooltip
	_reload_tooltip = _create_tooltip("Reload from last checkpoint", reload_btn)
...
func _play_intro() -> void:
	...
	tween.tween_callback(func():
		...
		create_tween().tween_property(reload_btn, "modulate:a", 1.0, 0.5)
	)
